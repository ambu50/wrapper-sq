...
_PARAM=$_PARAM' -o StrictHostKeyChecking=no -o ConnectTimeout=60 -o BatchMode=yes'

if [ "$_HOST" == "$_HOSTNAME" ] || [ "$_HOST" == "$_LOCALIP" ] || [ "$_HOST" == "$_LOCALHOST" ]; then
        ~/singularity/command $_COMMAND
        exit $?
else
	_PARAM=`echo $_PARAM | sed -e 's/-q//g'`
        _COMMAND=`echo $_COMMAND | sed -e 's/;/\\\;/g' | tr '"' "'"`
        /usr/bin/ssh_real $_PARAM $_HOST ~/singularity/job "$_COMMAND" 2>&1
        exit $?
fi
